	...
	public class Receiver extends BroadcastReceiver {
		...
		static void updateAutoAnswer(int status) {
			if (status != autoAnswerState) {
				...
				case 0:
					Receiver.onText(Receiver.AUTO_ANSWER_NOTIFICATION,"自动禁用",0,0);
					break;
				case 1:
					Receiver.onText(Receiver.AUTO_ANSWER_NOTIFICATION,"自动启用",0,0);
					break;
				case -1:
		...
		public static void pos(boolean enable) {
			...
			pos_gps(false);
			if (enable) {
				if (call_state == UserAgent.UA_STATE_IDLE && MainActivity.on(mContext) &&
		...
		*/}
		...
		public void onReceive(Context context, Intent intent) {
	        ...
	        if (!MainActivity.on(context)) return;
