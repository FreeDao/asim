...
public class SipdroidEngine implements RegisterAgentListener {
	public static final String TAG = "SipdroidEngine";
	...
	public boolean StartEngine() {
		...
		//			SipStack.log_path = "/data/data/org.sipdroid.sipua";
					...
					String version = "asim/" + Build.MODEL;
	...
	public void unregister(int i) {
			...
			if (ra != null && ra.unregister()) {
				...
				Receiver.onText(Receiver.REGISTER_NOTIFICATION+i, "◊¢≤·÷–...",R.drawable.sym_presence_idle,0);
				wl[i].acquire();
			} else
				Receiver.onText(Receiver.REGISTER_NOTIFICATION+i, null, 0, 0);
	}
	...
	public void onUaRegistrationSuccess(RegisterAgent reg_ra, NameAddress target,
		...
		if (isRegistered(i)) {
			...
			Receiver.onText(Receiver.REGISTER_NOTIFICATION+i,i == pref?"“—◊¢≤·":"µ„ª˜—°‘Ò",R.drawable.sym_presence_available,0);
	...
	public void onUaRegistrationFailure(RegisterAgent reg_ra, NameAddress target,
    	...
    	if (isRegistered(i)) {
    		...
    		retry = true;
    		Receiver.onText(Receiver.REGISTER_NOTIFICATION+i,"◊¢≤· ß∞‹"+" ("+result+")",R.drawable.sym_presence_away,0);
    	}
    	if (retry) {
    		retry = false;
	...
	public boolean togglemute() {
		boolean muted = ua.muteMediaApplication();
		Log.d(TAG, "mute:" + muted);
		if (muted)
			Receiver.onText(Receiver.CALL_NOTIFICATION, "æ≤“Ù", android.R.drawable.stat_notify_call_mute,Receiver.ccCall.base);
		else
			Receiver.progress();
